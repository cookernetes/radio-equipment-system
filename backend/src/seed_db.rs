...
pub async fn seed_db(db: &Database) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Encode username & pass hash to &[u8]
    let token_raw = UserRawPhysicalToken {
        username: initial_username.clone(),
        pass_hash: pass_hash.clone()
    };
    let token_raw = serde_json::to_string(&token_raw)?;
    ...
    if users_count == 0 && locations_count == 0 && items_count == 0 {
        ...
        let user_insert_result = users_coll.insert_one(User {
            id: None,
            username: initial_username.clone(),
            pass_hash,
        ...
        let _ = items_coll.insert_one(Item {
            ...
            quantity: 0,
            borrower_ids: vec![
                user_insert_result.inserted_id.as_object_id().unwrap()
            ],
